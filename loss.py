...
class PrototypeGuidanceLoss(nn.Module):
    def __init__(self, margin=0.5, align_weight=1.0, intra_weight=1.0, inter_weight=0.5):
        super(PrototypeGuidanceLoss, self).__init__()
        self.margin = margin
    ...
    def forward(self, feats, labels, modals):
        """
        feats:  [B, C]
        labels: [B]
        modals: [B]   RGB=0, IR=1
        ...
        for pid in uniq_ids:
            ...
            rgb_mask = pid_mask & (modals == 0)
            ir_mask  = pid_mask & (modals == 1)

            rgb_center = None
            ir_center = None

            if rgb_mask.sum() > 0:
                rgb_feat = feats[rgb_mask]
                rgb_center = rgb_feat.mean(dim=0, keepdim=True)   # [1, C]
                loss_intra = loss_intra + F.l1_loss(rgb_feat, rgb_center.expand_as(rgb_feat))
                num_intra += 1

            if ir_mask.sum() > 0:
                ir_feat = feats[ir_mask]
                ir_center = ir_feat.mean(dim=0, keepdim=True)     # [1, C]
                loss_intra = loss_intra + F.l1_loss(ir_feat, ir_center.expand_as(ir_feat))
                num_intra += 1

            if (rgb_center is not None) and (ir_center is not None):
                paired_rgb.append(rgb_center)
                paired_ir.append(ir_center)

        if num_intra > 0:
            loss_intra = loss_intra / num_intra

        if len(paired_rgb) > 0:
            paired_rgb = torch.cat(paired_rgb, dim=0)   # [N, C]
            paired_ir = torch.cat(paired_ir, dim=0)     # [N, C]

            # 同一 identity 的 RGB / IR prototype 对齐
            loss_align = F.l1_loss(paired_rgb, paired_ir)

            # 不同 identity prototype 拉开
            if paired_rgb.size(0) > 1:
                dist_rgb_ir = self.pairwise_dist(paired_rgb, paired_ir)   # [N, N]
                eye = torch.eye(dist_rgb_ir.size(0), device=device).bool()
                neg_dist = dist_rgb_ir[~eye]
                loss_inter = F.relu(self.margin - neg_dist).mean()
            else:
        ...
        loss = (
            ...
            self.intra_weight * loss_intra +
            self.inter_weight * loss_inter
        )
