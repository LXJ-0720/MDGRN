...
if dataset == 'llcm':
    ...
    tvsearch_flag = str(args.tvsearch).lower() == "true"

    for trial in range(10):
        test_trial = trial + 1

        if args.resume:
            resume_name = args.resume.format(trial=test_trial)
        else:
            resume_name = 'regdb_deen_p{}_n{}_lr_{}_seed_{}_trial_{}_ALL.t'.format(
                args.num_pos, args.batch_size, args.lr, args.seed, test_trial
            )
        model_path = os.path.join(checkpoint_path, resume_name)
        ...
        # training set, mainly for consistency with original code
        trainset = RegDBData(data_path, test_trial, transform=transform_train)
        color_pos, thermal_pos = GenIdx(trainset.train_color_label, trainset.train_thermal_label)

        # For RegDB, first load thermal and visible test sets.
        thermal_img, thermal_label = process_test_regdb(
            data_path,
            trial=test_trial,
            modal='thermal'
        )

        visible_img, visible_label = process_test_regdb(
            data_path,
            trial=test_trial,
            modal='visible'
        )
        ...
        thermal_loader = data.DataLoader(
            thermal_set,
        ...
        visible_loader = data.DataLoader(
            visible_set,
            batch_size=args.test_batch,
            shuffle=False,
            num_workers=args.workers
        ...
        nquery = len(thermal_label)
        ngall = len(visible_label)
        thermal_feat1, thermal_feat2, thermal_feat3, thermal_feat4, thermal_feat5, thermal_feat6 = \
            extract_query_feat(thermal_loader)

        # Extract visible features.
        nquery = len(visible_label)
        ngall = len(thermal_label)
        visible_feat1, visible_feat2, visible_feat3, visible_feat4, visible_feat5, visible_feat6 = \
            extract_gall_feat(visible_loader)

        if tvsearch_flag:
            print("True: thermal query -> visible gallery")

            distmat1 = np.matmul(thermal_feat1, np.transpose(visible_feat1))
            distmat2 = np.matmul(thermal_feat2, np.transpose(visible_feat2))
            distmat3 = np.matmul(thermal_feat3, np.transpose(visible_feat3))
            distmat4 = np.matmul(thermal_feat4, np.transpose(visible_feat4))
            distmat5 = np.matmul(thermal_feat5, np.transpose(visible_feat5))
            distmat6 = np.matmul(thermal_feat6, np.transpose(visible_feat6))

            a = 0.1
            distmat7 = distmat1 + distmat3 + distmat5 + distmat2 + distmat4 + distmat6
            distmat8 = a * (distmat1 + distmat3 + distmat5) + \
                       (1 - a) * (distmat2 + distmat4 + distmat6)

            cmc7, mAP7, mINP7 = eval_regdb(
                -distmat7,
                thermal_label,
                visible_label
            )

            cmc8, mAP8, mINP8 = eval_regdb(
                -distmat8,
                thermal_label,
                visible_label
            )
            ...
            print("False: visible query -> thermal gallery")

            distmat1 = np.matmul(visible_feat1, np.transpose(thermal_feat1))
            distmat2 = np.matmul(visible_feat2, np.transpose(thermal_feat2))
            distmat3 = np.matmul(visible_feat3, np.transpose(thermal_feat3))
            distmat4 = np.matmul(visible_feat4, np.transpose(thermal_feat4))
            distmat5 = np.matmul(visible_feat5, np.transpose(thermal_feat5))
            distmat6 = np.matmul(visible_feat6, np.transpose(thermal_feat6))
