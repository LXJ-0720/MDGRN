...
from utils import *
from loss import OriTripletLoss, CPMLoss, PrototypeGuidanceLoss
...
parser.add_argument('--arch', default='resnet50', type=str, help='network all:resnet18 or resnet50')
...
parser.add_argument('--lambda_proto', default=0, type=float, help='lambda for prototype guidance loss')
...
sys.stdout = Logger(log_path + suffix + '_ALL.txt')
...
for epoch in range(start_epoch, 151 - start_epoch):
    ...
    if epoch > 0 and epoch % 1 == 0:
        ...
        if cmc7[0] >= best_acc:  # not the real best for sysu-mm01
            ...
            torch.save(state, checkpoint_path + suffix + '_ALL.t')
